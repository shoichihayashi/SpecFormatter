...
class Formatter:
    def __init__(self, source_file, target_file):
        self.source_doc = Document(source_file)
        self.target_doc = Document(target_file)
    # extract formatting from paragraph
    def extract_formatting(self, run):
        formatting = {}
    ...
    def extract_list_formatting(self, paragraph):
        list_formatting = {}
    ...
    def apply_list_formatting(self, paragraph, list_formatting):
        if list_formatting:
    ...
    def insert_page_number(self, paragraph):
    ...
    def contains_page_number(self, paragraph):
        for run in paragraph.runs:
    ...
    def extract_header(self):
        ...
        if len(self.source_doc.sections) > 0:
            return self.source_doc.sections[0].header
        else:
            raise ValueError("The document has no header.")

    # extract footer
    def extract_footer(self):
        # Ensure the document has sections
        if len(self.target_doc.sections) > 0:
            return self.target_doc.sections[0].footer
        else:
            raise ValueError("The document has no footer.")
        
    # Clear and apply header to the target document
    def apply_header(self):
        source_header = self.extract_header()
        target_header = self.target_doc.sections[0].header
        ...
        for paragraph in source_header.paragraphs:
            ...
            for source_run, new_run in zip(paragraph.runs, new_paragraph.runs):
                formatting = self.extract_formatting(source_run)
                self.apply_formatting(new_run, formatting)
            # Check if the source footer contains the page number and add it to the target
            if self.contains_page_number(paragraph):
                self.insert_page_number(new_paragraph)

    # Clear and apply footer to the target document
    def apply_footer(self):
        source_footer = self.extract_footer()
        target_footer = self.target_doc.sections[0].footer
    ...
    # save file
    def save_target(self, output_name):
        self.target_doc.save(output_name)

    
    # apply formatting for paragraphs, including list formatting
    def apply_paragraph_formatting(self, source_paragraph, target_paragraph):

        # check if source paragraph is list
        if self.is_list_paragraph(source_paragraph):
            # extract and apply list formattings
            list_formatting = self.extract_list_formatting(source_paragraph)
            self.apply_list_formatting(target_paragraph, list_formatting)
        ...
        if source_paragraph.runs:
            for source_run, target_run in zip(source_paragraph.runs, target_paragraph.runs):
                formatting = self.extract_formatting(source_run)
                self.apply_formatting(target_run, formatting)
